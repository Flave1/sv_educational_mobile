...
export const getStudentnotes = (classId: string, subjectId: string, status: number = -2, pageNumber: number = 1) => (dispatch: any) => {
    Device.isInternetAvailable().then((hasInternetAccess: boolean) => {
        if (hasInternetAccess) {
            ...
            axiosInstance.get(`smp/server/smp/studentnotes/api/v1/get/studentnotes/by-teacher/mobile?classId=${classId}&subjectId=${subjectId}&status=${status}&pageNumber=${pageNumber}`)
                .then((res) => {
